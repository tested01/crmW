...
export default class extends Component {
  render () {
    ...
    for (var i = 0; i < this.props.works.length; i++) {
      rows.push(<View key={i} style={styles.slide} title={<Text allowFontScaling={false} numberOfLines={1}></Text>}>
    ...
    return (
      <View style={{marginBottom: 10}}>
        ...
        <View style={{flex: 1,
          ...
          <Text allowFontScaling={false} style={{textAlign: 'center', color: 'skyblue'}}>{this.props.cardName}</Text>
          <Text allowFontScaling={false} style={{textAlign: 'center'}}>{this.props.cardContent}</Text>
          </View>
        </View>
      </View>
    )
